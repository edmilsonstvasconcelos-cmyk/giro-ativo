...
import { ArrowLeft, Calendar, CheckCircle, FileDown, AlertTriangle, Package } from 'lucide-react'
...
export default async function LaudoDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Garante que o laudo pertence a um produto do vendedor logado
  ...
  const statusColor = statusColors[laudo.status] ?? statusColors.solicitado

  return (
    <div className="max-w-2xl space-y-5">
      {/* Header */}
      <div className="flex items-center gap-3">
        <Link href="/vendedor/laudos" className="p-2 rounded-lg hover:bg-muted transition-colors">
        ...
        <span className={`text-xs px-3 py-1 rounded-full border font-semibold shrink-0 ${statusColor}`}>
      ...
      <div className="rounded-xl border border-border bg-card p-4 flex gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground capitalize mt-0.5">{laudo.modalidade}</p>
          <Link href={`/produtos/${produto?.id}`} className="text-xs text-primary hover:underline">
            Ver no marketplace
          </Link>
        </div>
      </div>
      ...
      {/* Avaliador (quando atribuído) */}
      {avaliador && (
        <div className="rounded-xl border border-border bg-card p-5">
          <h2 className="font-semibold mb-2">Avaliador responsável</h2>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl gradient-brand/10 border border-primary/20 flex items-center justify-center text-primary font-bold text-sm shrink-0">
              {(avaliador.nome_fantasia ?? avaliador.razao_social ?? 'A')[0]}
            </div>
      ...
      {laudo.status === 'concluido' && (
        <div className="rounded-xl border border-green-200 bg-green-50 dark:bg-green-500/10 dark:border-green-500/20 p-5 space-y-4">
          <h2 className="font-semibold text-green-800 dark:text-green-300">Laudo emitido</h2>
          ...
          {laudo.valor_mercado_sugerido && (
            <div>
              ...
              <p className="font-semibold">
                R$ {(laudo.valor_mercado_sugerido as number).toLocaleString('pt-BR')}
              </p>
            </div>
          )}
          ...
          {laudo.validade_ate && (
            <div className={`flex items-center gap-2 text-sm font-medium ${
              ...
              {expired
                ? `Laudo vencido em ${format(new Date(laudo.validade_ate), 'dd/MM/yyyy')}`
                : expiresIn !== null && expiresIn <= 7
                  ? `Atenção: vence em ${expiresIn} dia${expiresIn === 1 ? '' : 's'}`
      ...
      {laudo.observacoes && (
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <p className="text-sm text-muted-foreground leading-relaxed whitespace-pre-wrap">
            {laudo.observacoes as string}
          </p>
        </div>
      )}

      {/* Agendamento */}
      {laudo.data_agendada && !['concluido', 'cancelado', 'expirado'].includes(laudo.status) && (
        <div className="rounded-xl border border-border bg-card p-5">
          <h2 className="font-semibold mb-2">Vistoria agendada</h2>
          <div className="flex items-center gap-2 text-sm">
            <Calendar className="w-4 h-4 text-primary" />
            <span>
              {format(new Date(laudo.data_agendada + 'T00:00:00'), "dd 'de' MMMM 'de' yyyy", { locale: ptBR })}
              {laudo.hora_agendada ? ` às ${(laudo.hora_agendada as string).slice(0, 5)}` : ''}
            </span>
          </div>
        </div>
